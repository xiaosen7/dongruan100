  ...
  render() {
    ...
    return (
      <div className='questionItem' onClick={this.onSelect}>
        ...
        <img className='time-border' alt='边框' src={require('../static/img/question/time-border.png')} />
        <span className='count' ref="count">0</span>
        <a href="###" onClick={this.pause} className="controlMusic">
          <img alt='声音控制' src={require('../static/img/question/sun.png')} ref={'sun'} />
        </a>
        <img alt='建筑' style={{ animation: `${a1} 1s` }} src={require('../static/img/question/building.png')} />
        <img alt='小装饰' style={{ animation: `${a2} 1s` }} src={require('../static/img/question/question-border.png')} />
        <img alt='问题' style={{ animation: `${a3} 1s` }} src={require(`../static/${question}`)} />
        <img alt='小装饰' style={{ animation: `${a4} 1s` }} src={require('../static/img/question/decoration.png')} />
        ...
        {picAry.map((item, index) => {
          if (item) {
            if (index === 2 && answersLen === 3) {
              return <img data-pic={ABCD[index]} alt={`选择${ABCD[index]}`} style={{ ...styles.question[4], animation: `${a5} 1s` }}
                src={require(`../static/${item}`)}
                key={index} />
            }
            return <img data-pic={ABCD[index]} alt={`选择${ABCD[index]}`} style={{ ...styles.question[index], animation: `${a5} 1s` }}
              src={require(`../static/${item}`)}
              key={index} />
          }
          return null;
